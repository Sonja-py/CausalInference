...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.6ea72d06-34cf-4f3b-bbe2-3674a0164287"),
    final_data=Input(rid="ri.foundry.main.dataset.189cbacb-e1b1-4ba8-8bee-9d6ee805f498"),
...
def Rerun_best_for_auc_lrt(final_data, test_lr_tlearner):
    def load_model(fs, names):
        for name in names:
            try:
                with fs.open(f"{name}.pickle", "rb") as f:
    ...
    fs = test_lr_tlearner.filesystem()
    results = []
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.07ffc213-d9db-4cff-a36d-2d89e5a8bb9d"),
    final_data=Input(rid="ri.foundry.main.dataset.189cbacb-e1b1-4ba8-8bee-9d6ee805f498"),
...
def Rerun_best_for_auc_rft(final_data, test_rf_tlearner):
    def load_model(fs, names):
        for name in names:
            try:
                with fs.open(f"{name}.pickle", "rb") as f:
    ...
    fs = test_rf_tlearner.filesystem()
    results = []
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.36a6647a-ec1a-4dd0-bcd8-21571858cd7a"),
    Test_rf_slearner=Input(rid="ri.foundry.main.dataset.d8a3ce5b-5472-4704-9d3a-205920048c80"),
    final_data=Input(rid="ri.foundry.main.dataset.189cbacb-e1b1-4ba8-8bee-9d6ee805f498")
)
import pandas as pd
...
def rf_slearner_predictions_y0y1(final_data, Test_rf_slearner):
...
@transform_pandas(
    Output(rid="ri.foundry.main.dataset.f6812ea9-60fb-404d-b7ec-a72a824b885a"),
    Test_rf_tlearner=Input(rid="ri.foundry.main.dataset.3cbc3c8c-65b6-4f67-8c4e-40bc4da8bbe8"),
    final_data=Input(rid="ri.foundry.main.dataset.189cbacb-e1b1-4ba8-8bee-9d6ee805f498")
)
import pandas as pd
...
def splittest(final_data, test_lr_slearner):
    # Call y0y1 to get test sets
    X_train_1, X_test_1, t_train_1, t_test_1, y_train_1, y_test_1 = y0y1(final_data, test_lr_slearner)
    
    # Call bootstrap to get test sets
    X_train_2, X_test_2, t_train_2, t_test_2, y_train_2, y_test_2 = bootstrap(final_data, test_lr_slearner)
    
    # Compare test sets for X
    X_test_equal = X_test_1.equals(X_test_2)
    # Compare treatment vectors in test sets
    t_test_equal = t_test_1.reset_index(drop=True).equals(t_test_2.reset_index(drop=True))
    # Compare labels in test sets
    y_test_equal = y_test_1.reset_index(drop=True).equals(y_test_2.reset_index(drop=True))
    
    print("Comparison of test splits:")
