...
def create_ate_df(roc, combination, model, parameters):
    things = {}
    # things["ATE"] = ate
    things["ROC"] = roc
...
def cevae_func(param, X_train_val_without_treatment, y_train_val, treatment_series, combination, results_df):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_val_without_treatment, y_train_val)):
        ...
        for i in range(count):
            ...
            ites, y0, y1 = cevae.ite(X_val_fold)
        ...
        fold_scores.append(roc_total/count)
        
        del cevae
        torch.cuda.empty_cache()
    mean_scores = np.mean(fold_scores)
    out_df = create_ate_df(mean_scores, combination, "CEVAE", param)
