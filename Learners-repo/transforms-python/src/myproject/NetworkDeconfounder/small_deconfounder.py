...
@transform(
    output_df=Output("ri.foundry.main.dataset.0b518dae-06f5-4ed9-9df0-f446fada886b"),
    output_loss=Output("ri.foundry.main.dataset.3386ba9e-f237-4f8c-9df8-fda451ecf248"),
    output_num=Output("ri.foundry.main.dataset.6bd9984e-6ad5-4ea2-a92c-2152c8d0e92b"),
...
def compute(
    ...
    results_df, out_loss, num_people = compute_network_deconfounder(
        final_data,
        mini_batches[0],
    ...
    df_loss = spark.createDataFrame(pd.DataFrame.from_dict(out_loss, orient="index"))
    df_num = spark.createDataFrame(pd.DataFrame([[num_people]]))
    num = output_num.write_dataframe(df_num)
    x = output_loss.write_dataframe(df_loss)
