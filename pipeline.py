...
def meta_learners_t(final_data):
    ...
    for idx, combination in enumerate(ingredient_pairs):
        ...
        ite, yhat_cs, yhat_ts = learner_t1.predict(X=X_test, treatment=t_test, y=y_test, return_components=True, verbose=True)
        roc, ate = metrics(y_test, t_test, ite, yhat_cs, yhat_ts, threshold, 'RandomForestClassifier')
        rocs_r.append(roc)
        ates_r.append(ate)
        ...
        ite, yhat_cs, yhat_ts = learner_t2.predict(X=X_test, treatment=t_test, y=y_test, return_components=True, verbose=True)
        roc, ate = metrics(y_test, t_test, ite, yhat_cs, yhat_ts, threshold, 'LogisticRegression')
        rocs_l.append(roc)
        ates_l.append(ate)
